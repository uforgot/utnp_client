...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  // ===== 중앙 ↔ 바깥 전이 =====
  const float INNER = 0.35; // 중앙 원 크기
  const float EDGE  = 0.25; // 전이 폭(넓힐수록 부드럽게)
  ...
  const float HUE_TIME   = 0.1; // 시간 속도 ↓ (천천히 회전)
  const float HUE_SPACE  = 5.0;  // 반지름 따라 색 변화 ↓ (덜 돌게)
  const float HUE_AUDIO  = 0.08;  // 오디오 영향 ↓
  ...
  // ===== 밝기 =====
  const float RING_FREQ  = 0.1;  // 링 간격 ↓
  const float RING_SPEED = 0.1;   // 링 이동 속도 ↓
  float ringFall = smoothstep(0.4, 0.0, s);
  float pulse = sin(RING_FREQ * s - iTime * RING_SPEED + sound * 0.1) * 0.5 + 0.5;
