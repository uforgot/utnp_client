...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  float alphaFromBlack = clamp(max(max(finalColor.r, finalColor.g), finalColor.b), 0.0, 1.0);
  float outsideCut = scaledR < 1.0 ? 1.0 : 0.0;
  float alpha2 = alphaFromBlack * outsideCut;
  fragColor = vec4(finalColor, alpha2);
